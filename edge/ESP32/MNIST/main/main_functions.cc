...
#define BUF_SIZE 784  // Adjust the buffer size as needed
...
#define COLUMNS 28
#define CHANNELS 1
uint8_t image[CHANNELS][ROWS][COLUMNS];
...
void loop() {
  ...
  if (rx_length > 0) {
    ...
    // *****************  INFERENCE ****************
    // copy the image array into the input tensor.
    memcpy(input->data.uint8, rx_buffer, sizeof(rx_buffer));
    ...
    // first 10 values being the de-quantized output of the model, the 11th value being the logistic regression result,
    ...
    uint8_t label = 0; // index of the maximum value in the output tensor
    float max_value = -FLT_MAX; // maximum value in the output tensor
    for (int i = 0; i < 10; ++i) {
      scores[i] = output->data.uint8[i] * 0.00390625; // Dequantize the output
    ...
    // convert the uint8_t index to a string and add it to the output string
    char tempBuffer[20];
    snprintf(tempBuffer, sizeof(tempBuffer), "%d,", label);
